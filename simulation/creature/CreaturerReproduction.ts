...
export default class CreatureReproduction {
    ...
    get hasEnoughtMassToReproduce() {
        return this._creature.mass.mass * 2 > this._creature.mass.massAtBirth + this._creature.mass.mass* (1+constants.REPRODUCTION_COST_PER_MASS_DO);
    }
    ...
    reproduce(input: number) : boolean {
      ...
      if (!this.hasEnoughtMassToReproduce) {
          this._creature.mass.consumeMassFraction(constants.REPRODUCTION_COST_PER_MASS_TRY);    
          return false;
      }
      ...
      if (offspringPosition) {
        this._creature.generations.newCreature(offspringPosition, this._creature.mass.massAtBirth, this._creature.brain.genome);
        this._creature.mass.consumeMassFraction(constants.REPRODUCTION_COST_PER_MASS_DO);    
        return true;
      }
      this._creature.mass.consumeMassFraction(constants.REPRODUCTION_COST_PER_MASS_TRY);    
      return false;
